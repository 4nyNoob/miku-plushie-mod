...
public class MeikoRender extends GeoEntityRenderer<MeikoEntity> {
    ...
    public MeikoRender(EntityRendererFactory.Context renderManager) {
        super(renderManager, new MeikoModel());

        // Add some held item rendering
        addRenderLayer(new BlockAndItemGeoLayer<>(this) {
            @Nullable
            public ItemStack getStackForBone(GeoBone bone, MeikoEntity animatable) {
                // Retrieve the items in the entity's hands for the relevant bone
                return switch (bone.getName()) {
                    case LEFT_HAND -> animatable.isLeftHanded() ?
                        MeikoRender.this.mainHandItem : MeikoRender.this.offHandItem;
                    case RIGHT_HAND -> animatable.isLeftHanded() ?
                        MeikoRender.this.offHandItem : MeikoRender.this.mainHandItem;
                    default -> null;
                };
            }

            public ModelTransformationMode getTransformTypeForStack(GeoBone bone, ItemStack stack, MeikoEntity animatable) {
            ...
            public void renderStackForBone(MatrixStack poseStack, GeoBone bone, ItemStack stack, MeikoEntity animatable,
                                            VertexConsumerProvider bufferSource, float partialTick, int packedLight, int packedOverlay) {
                if (stack == MeikoRender.this.mainHandItem) {
                    poseStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-90f));

                    if (stack.getItem() instanceof ShieldItem)
                        poseStack.translate(0, 0.125, -0.25);
                }
                else if (stack == MeikoRender.this.offHandItem) {
                    poseStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-90f));
