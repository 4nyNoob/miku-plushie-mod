...
public class NeruRender extends GeoEntityRenderer<NeruEntity> {
    ...
    public NeruRender(EntityRendererFactory.Context renderManager) {
        super(renderManager, new NeruModel());

        // Add some held item rendering
        addRenderLayer(new BlockAndItemGeoLayer<>(this) {
            @Nullable
            public ItemStack getStackForBone(GeoBone bone, NeruEntity animatable) {
                // Retrieve the items in the entity's hands for the relevant bone
                return switch (bone.getName()) {
                    case LEFT_HAND -> animatable.isLeftHanded() ?
                        NeruRender.this.mainHandItem : NeruRender.this.offHandItem;
                    case RIGHT_HAND -> animatable.isLeftHanded() ?
                        NeruRender.this.offHandItem : NeruRender.this.mainHandItem;
                    default -> null;
                };
            }

            public ModelTransformationMode getTransformTypeForStack(GeoBone bone, ItemStack stack, NeruEntity animatable) {
            ...
            public void renderStackForBone(MatrixStack poseStack, GeoBone bone, ItemStack stack, NeruEntity animatable,
                                            VertexConsumerProvider bufferSource, float partialTick, int packedLight, int packedOverlay) {
                if (stack == NeruRender.this.mainHandItem) {
                    poseStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-90f));

                    if (stack.getItem() instanceof ShieldItem)
                        poseStack.translate(0, 0.125, -0.25);
                }
                else if (stack == NeruRender.this.offHandItem) {
                    poseStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-90f));
