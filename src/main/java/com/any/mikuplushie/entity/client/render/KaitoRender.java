...
public class KaitoRender extends GeoEntityRenderer<KaitoEntity> {
    ...
    public KaitoRender(EntityRendererFactory.Context renderManager) {
        super(renderManager, new KaitoModel());

        // Add some held item rendering
        addRenderLayer(new BlockAndItemGeoLayer<>(this) {
            @Nullable
            public ItemStack getStackForBone(GeoBone bone, KaitoEntity animatable) {
                // Retrieve the items in the entity's hands for the relevant bone
                return switch (bone.getName()) {
                    case LEFT_HAND -> animatable.isLeftHanded() ?
                        KaitoRender.this.mainHandItem : KaitoRender.this.offHandItem;
                    case RIGHT_HAND -> animatable.isLeftHanded() ?
                        KaitoRender.this.offHandItem : KaitoRender.this.mainHandItem;
                    default -> null;
                };
            }

            public ModelTransformationMode getTransformTypeForStack(GeoBone bone, ItemStack stack, KaitoEntity animatable) {
            ...
            public void renderStackForBone(MatrixStack poseStack, GeoBone bone, ItemStack stack, KaitoEntity animatable,
                                            VertexConsumerProvider bufferSource, float partialTick, int packedLight, int packedOverlay) {
                if (stack == KaitoRender.this.mainHandItem) {
                    poseStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-90f));

                    if (stack.getItem() instanceof ShieldItem)
                        poseStack.translate(0, 0.125, -0.25);
                }
                else if (stack == KaitoRender.this.offHandItem) {
                    poseStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-90f));
