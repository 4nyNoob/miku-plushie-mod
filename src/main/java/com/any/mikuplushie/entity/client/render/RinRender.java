...
public class RinRender extends GeoEntityRenderer<RinEntity> {
    ...
    public RinRender(EntityRendererFactory.Context renderManager) {
        super(renderManager, new RinModel());

        // Add some held item rendering
        addRenderLayer(new BlockAndItemGeoLayer<>(this) {
            @Nullable
            public ItemStack getStackForBone(GeoBone bone, RinEntity animatable) {
                // Retrieve the items in the entity's hands for the relevant bone
                return switch (bone.getName()) {
                    case LEFT_HAND -> animatable.isLeftHanded() ?
                        RinRender.this.mainHandItem : RinRender.this.offHandItem;
                    case RIGHT_HAND -> animatable.isLeftHanded() ?
                        RinRender.this.offHandItem : RinRender.this.mainHandItem;
                    default -> null;
                };
            }

            public ModelTransformationMode getTransformTypeForStack(GeoBone bone, ItemStack stack, RinEntity animatable) {
            ...
            public void renderStackForBone(MatrixStack poseStack, GeoBone bone, ItemStack stack, RinEntity animatable,
                                            VertexConsumerProvider bufferSource, float partialTick, int packedLight, int packedOverlay) {
                if (stack == RinRender.this.mainHandItem) {
                    poseStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-90f));

                    if (stack.getItem() instanceof ShieldItem)
                        poseStack.translate(0, 0.125, -0.25);
                }
                else if (stack == RinRender.this.offHandItem) {
                    poseStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-90f));
